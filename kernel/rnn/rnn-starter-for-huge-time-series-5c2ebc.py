...
from numpy import quantile, sum    
...
#float_data = pd.read_csv("/Users/kennyhelsens/git/itp-earthquake-prediction-ml/data/train/train.csv", dtype={"acoustic_data": np.float32, "time_to_failure": np.float32}).values
...
def extract_features(z):
     return np.c_[z.mean(axis=1), 
                  ...
                  z.max(axis=1),
                  z.std(axis=1),
                  quantile(z, 0.9, axis=1),
                  quantile(z, 0.1, axis=1),
                  quantile(z, 0.99, axis=1),
                  quantile(z, 0.01, axis=1)
                  ]
...
from keras.layers import Dense, CuDNNGRU,Dropout,LSTM
from keras.optimizers import adam
from keras.callbacks import ModelCheckpoint

cb = [ModelCheckpoint("model.h5", save_best_only=True, period=3)]

model = Sequential()
model.add(CuDNNGRU(128, input_shape=(None, n_features)))
...
def perf_plot(history, what = 'loss'):
    ...
    plt.plot(epochs, x, 'b',color="red", label = "Training " + what)
